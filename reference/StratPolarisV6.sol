/**
 *Submitted for verification at aurorascan.dev on 2022-04-10
 */
...
// File contracts/interfaces/common/IPolarisRewarder.sol
...
// File contracts/interfaces/common/ITriRewarder.sol
...
contract StratManager is Ownable, Pausable {
     ...
     * {keeper2} - Address to manage a few lower risk features of the strat
     * {harvester} - Address that calls autocomp functions
     * {treasury} - Address of the strategy author/deployer where treasury fee will go.
     * {vault} - Address of the vault that controls the strategy's funds.
     * {unirouter} - Address of exchange to execute swaps.
     */
    address public keeper = 0x6d62ED9470Eb0fcfe2C17493ac32B555be44E2Cd;
    address public keeper2 = 0xCbA5D325fa06078d8c55c122ab43f758cc11A4f7;
    address public harvester = 0x78535BA8cAACa07f0D16077De8dc46BDF063FF02; //  harvester1
    address public treasury = 0x6159B3D96CE5F7dCC07FeB17C4A76385159f92Fc;
    address public unirouter;
    address public vault = zero;
    address public vaporwaveFeeRecipient = 0x271Cb05a2e56f2FbF655bDD0480f035C578a92e0; //  v2 with notify
    ...
    modifier onlyManager() {
        require(
            msg.sender == owner() || msg.sender == keeper || msg.sender == keeper2 || msg.sender == harvester,
            "!manager"
        );
        _;
    }
     ...
     * @param _harvester new keeper address.
     */
     ...
     */
    function setKeeper1(address _keeper) external {
    ...
    /**
     * @dev Updates address of the strat keeper.
     * @param _keeper2 new keeper2 address.
     */
    function setKeeper2(address _keeper2) external {
        require(msg.sender == keeper2, "!NotKeeper2!");
        keeper2 = _keeper2;
    }
     ...
     * can only be called once, so set to onlyManager
     */
    function setVault(address _vault) external onlyManager {
        require(vault == zero, "nope");
        vault = _vault;
    }

    /**
     * @dev Updates Vaporwave fee recipient.
     * @param _vaporwaveFeeRecipient new Vaporwave fee recipient address.
     */
    function setVaporwaveFeeRecipient(address _vaporwaveFeeRecipient) external onlyOwner {
        vaporwaveFeeRecipient = _vaporwaveFeeRecipient;
    }
     ...
     * Can be overridden in the strategy.
     */
    function beforeWithdraw() external virtual {}
}
...
abstract contract FeeManager is StratManager {
    uint256 public constant TREASURY_FEE = 112;
    ...
    uint256 public withdrawalFee = 10; //  0.1% stays in pool for others

    uint256 public callFee = 111;
    uint256 public vaporwaveFee = MAX_FEE - TREASURY_FEE - callFee;

    function setCallFee(uint256 _fee) external onlyManager {
        require(_fee <= MAX_CALL_FEE, "!cap");

        callFee = _fee;
        vaporwaveFee = MAX_FEE - TREASURY_FEE - callFee;
    }
...
// File contracts/v6strats/StratPolarisV6.sol

pragma solidity ^0.6.0;

contract StratPolarisV6 is StratManager, FeeManager {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    // Tokens used
    address public constant wrapped = address(0xC9BdeEd33CD01541e1eeD10f90519d2C06Fe3feB); //  wrapped ETH.
    address public constant output = address(0x9D6fc90b25976E40adaD5A3EdD08af9ed7a21729); //  SPOLAR
    ...
    address public constant masterchef = address(0xA5dF6D8D59A7fBDb8a11E23FDa9d11c4103dc49f); //  spolar rewarder
    ...
    bool public harvestOnWithdrawal;
    ...
    event autoHarvestSet(bool isHarvDep, bool isHarvWith);
    event dualRewardSet(address dualReward);
    ...
    function deposit() public whenNotPaused {
        ...
        if (wantBal > 0) {
            IPolarisRewarder(masterchef).deposit(poolId, wantBal);
        }
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IPolarisRewarder(masterchef).withdraw(poolId, _amount.sub(wantBal));
    ...
    // set bool harvestOnDeposit and harvestOnWithdrawal
    function setAutoHarvest(bool _harvestOnDeposit, bool _harvestOnWithdrawal) external onlyManager {
        harvestOnDeposit = _harvestOnDeposit;
        harvestOnWithdrawal = _harvestOnWithdrawal;
        emit autoHarvestSet(harvestOnDeposit, harvestOnWithdrawal);
    }

    function beforeDeposit() external override {
        if (harvestOnDeposit) {
            require(msg.sender == vault, "!vault");
            _harvest(tx.origin); //  before harvest, call harvest, pass in argument of tx.origin
        }
    }

    function beforeWithdraw() external override {
        if (harvestOnWithdrawal) {
            require(msg.sender == vault, "!vault");
            _harvest(tx.origin); //  before withdrawal, call harvest, pass in argument of tx.origin
        }
    }
    ...
    //  call harvest, param public callFee, for frontend
    function harvest(address callFeeRecipient) external virtual {
        _harvest(callFeeRecipient);
    }

    //  call harvest, manager
    ...
    function _harvest(address callFeeRecipient) internal whenNotPaused {
        IPolarisRewarder(masterchef).deposit(poolId, 0);
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        if (toOutput > 0) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(
                toOutput,
                0,
                dualRewardToOutputRoute,
                address(this),
                now.add(600)
            );
        }

        uint256 toWrapped = IERC20(output).balanceOf(address(this)).mul(42).div(1000); //  wrapped amount   4.20% fees
        IUniswapRouterETH(unirouter).swapExactTokensForTokens(
            toWrapped,
            0,
        ...
        uint256 callFeeAmount = wrappedBal.mul(callFee).div(MAX_FEE); //  callFee amount = amount must go to the caller of the harvest function
        IERC20(wrapped).safeTransfer(callFeeRecipient, callFeeAmount); //  send callFee amount to tx.origin

        uint256 vaporwaveFeeAmount = wrappedBal.mul(vaporwaveFee).div(MAX_FEE); //  user rewards + VWAVE buybacks
        IERC20(wrapped).safeTransfer(vaporwaveFeeRecipient, vaporwaveFeeAmount);

        uint256 treasuryFee = wrappedBal.mul(TREASURY_FEE).div(MAX_FEE); //  VWAVE protocol profit fee
        IERC20(wrapped).safeTransfer(treasury, treasuryFee);
    }
    ...
    function pendingRewards() public view returns (uint256) {
        uint256 _amount = IPolarisRewarder(masterchef).pendingShare(poolId, address(this));
        return _amount;
    }
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount, ) = IPolarisRewarder(masterchef).userInfo(poolId, address(this));
        return _amount;
    }
    ...
    function callReward() public view returns (uint256) {
        uint256 outputPend = pendingRewards();
        uint256 nativeOut;
        if (outputPend > 0) {
            try IUniswapRouterETH(unirouter).getAmountsOut(outputPend, outputToWrappedRoute) returns (
        ...
        return nativeOut.mul(42).div(1000).mul(callFee).div(MAX_FEE);
    }
    ...
    function retireStrat() external {
        ...
        IPolarisRewarder(masterchef).emergencyWithdraw(poolId);
